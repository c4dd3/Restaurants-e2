...
func TestUserRepoMongoCRUD(t *testing.T) {
	...
	// Después de borrar, FindByID debe retornar ErrNotFound (no nil, nil).
	_, err = repos.Users.FindByID(ctx, "user-1")
	if !errors.Is(err, domain.ErrNotFound) {
		t.Fatalf("esperaba ErrNotFound para usuario eliminado, obtuvo %v", err)
	}
}
...
func TestUserRepoMongoDefaultsAndMissing(t *testing.T) {
	...
	_, err := repos.Users.FindByID(ctx, "no-existe")
	if !errors.Is(err, domain.ErrNotFound) {
		t.Fatalf("esperaba ErrNotFound para usuario inexistente, obtuvo %v", err)
	}

	// FindByEmail con email inexistente retorna ErrNotFound, no nil.
	_, err = repos.Users.FindByEmail(ctx, "nadie@example.com")
	if !errors.Is(err, domain.ErrNotFound) {
		t.Fatalf("esperaba ErrNotFound para email inexistente, obtuvo %v", err)
	}

	// Update con id inexistente retorna ErrNotFound.
	_, err = repos.Users.Update(ctx, "no-existe", &domain.UpdateUserRequest{Name: "Nada"})
	if !errors.Is(err, domain.ErrNotFound) {
		t.Fatalf("esperaba ErrNotFound al actualizar usuario inexistente, obtuvo %v", err)
	}

	// Borrar un id inexistente retorna ErrNotFound.
	if err := repos.Users.Delete(ctx, "no-existe"); !errors.Is(err, domain.ErrNotFound) {
		t.Fatalf("esperaba ErrNotFound al borrar usuario inexistente, obtuvo %v", err)
	}
}
